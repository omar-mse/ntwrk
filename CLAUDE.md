...
- `middleware.ts` — Wires `auth.config.ts` into the edge runtime via `NextAuth(authConfig).auth` and gates every non-static route through the `authorized` callback.
